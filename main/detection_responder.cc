...
void RespondToDetection(float* car_scores, char* kCategoryLabels[]) {
  // Find the car with the highest score.
  float max_score = 0;
  int max_score_index = 0;
  for (int i = 0; i < kCategoryCount; i++) {
    if (car_scores[i] > max_score) {
      max_score = car_scores[i];
      max_score_index = i;
    }
  }
  ...
  // Log the detected car.
  MicroPrintf("Detected car: %s", kCategoryLabels[max_score_index]);
  
  // Send the detected car to the server.
  post_rest_function(kCategoryLabels[max_score_index], max_score * 100);

  for (int i = 0; i < kCategoryCount; i++) {
    MicroPrintf("%s: %f\n", kCategoryLabels[i], car_scores[i]); 
  }
}
